...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="ppo-bpw",
...
def make_env(gym_id, seed, idx, capture_video, run_name):
    def thunk():
        if capture_video:
            ...
            env = gym.make(gym_id, hardcore=True)
        ...
        env = gym.wrappers.TransformReward(env, lambda reward: np.clip(reward, -400, 400))
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            for item_idx in range(len(done)):
                if done[item_idx]:
                    print(f"global_step={global_step}, episodic_return={sum(rewards[-step:])[item_idx]}")
                    writer.add_scalar("charts/episodic_return", sum(rewards[-step:])[item_idx], global_step)
