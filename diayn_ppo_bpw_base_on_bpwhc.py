...
def parse_args():
    ...
    # DIAYN
    parser.add_argument("--n_skills", default=20, type=int,
...
def make_env(gym_id, seed, capture_video, run_name, hardcore=False):
    def thunk():
        if capture_video and hardcore:
            env = gym.make(gym_id, render_mode="rgb_array", hardcore=True)
            env = gym.wrappers.RecordVideo(env, f"videos/{run_name}")
        elif capture_video and not hardcore:
...
def record_skill_videos(z, args, agent, run_name):
    ...
    env = make_env(args.gym_id, args.seed, args.capture_video, run_name, hardcore=True)()
    obs, _ = env.reset()
